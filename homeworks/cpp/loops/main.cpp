...
int main() {
    try {
        ...
        std::cout
            << "[+] Enter the range [a, b] (two integers separated by a space): ";
        if (!(std::cin >> a >> b)) {
        ...
        if (totalValue > 0) {
            std::cout << "[OK] Total value of signals in the range [" << a << ", "
                << b << "]: " << totalValue << std::endl;
        } else {
...
int dadad() {
    ...
    try {
        ...
        std::cerr << "[E] " << e.what() << std::endl;
        return 1; // coz 5 + 0 = 5 / 555e === 2 !== 0x10242011
    }
}
